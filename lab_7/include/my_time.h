...
class Time
{
    ...
    static void show_time_format_exception(const std::string &time, const std::exception &exc);
    Time(int input_hour, int input_minute, int input_second);
  ...
  public:
    ...
    {
        ostm << std::setfill('0') << std::setw(2) << time.hour << ':' << std::setw(2) << time.minute
